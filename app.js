...
app.get("/viewProject", async (req, res) => {
  try {
    let curProject = await Project.findById(req.query.project).populate({
      path: "issues",
    }); //fetching project by req.query,id
...
app.post("/viewProject", async (req, res) => {
  ...
  if (req.body.selectAuthor != 0) {
    ...
    if (findIssue)
      curProject = await Project.findById(findIssue.projectID).populate({
        path: "issues",
      });
  } else if (req.body.selectDescription != 0 && req.body.selectTitle != 0) {
    findIssue = await Issue.findOne({
      description: req.body.selectDescription,
      title: req.body.selectTitle,
    }); //fetching issue by auhtor & description
...
app.post("/createIssue", async (req, res) => {
  try {
    ...
    let update = await Project.findByIdAndUpdate(req.body.projectID, {
      $push: { issues: newIssue.id },
    });
    //finding project by project id

    return res
      .writeHead(301, {
        Location: `http://localhost:${PORT}/viewProject/?project=${req.body.projectID}`,
      })
      .end();
